...
class FaltasViewModel: ViewModel() {
    ...
    fun deleteFalta(id: Int) {
        viewModelScope.launch {
            try {

                if(serv.deleteFalta(id)){
                    Log.d("Falta_Asistencia", "Falta with id $id deleted successfully")
                } else {
                    Log.d("Falta_Asistencia", "Failed to delete Falta with id $id")
                }
            } catch (e: Exception) {
                Log.d("Falta_Asistencia", "$e")
                e.printStackTrace()
