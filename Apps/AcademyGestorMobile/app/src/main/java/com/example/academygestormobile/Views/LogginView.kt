...
@Composable
fun Loggin(nav: NavHostController, usuariosViewModel: UsuariosViewModel = viewModel()) {

    var context = LocalContext.current
    val usuario: Usuario? by usuariosViewModel.usuario.observeAsState(null)
    ...
    LaunchedEffect(Unit) {
        usuariosViewModel.resetUsuario()
        val sharedPreferences: SharedPreferences = context.getSharedPreferences("UserPrefs", Context.MODE_PRIVATE)
    ...
    Box(
        ...
        Column(
            ...
            LaunchedEffect(usuario) {
                Log.d("LogginView", "Usuario obtenido: $usuario")
    ...
    if (errorMessage) {
        AlertDialog(
            onDismissRequest = { errorMessage = false },
            title = { Text("Error de inicio de sesión") },
            text = { Text("Usuario o contraseña inválidos. Por favor, inténtalo de nuevo.") },
            confirmButton = {
                Button(onClick = { errorMessage = false }) {
                    Text("Aceptar")
                }
            }
        )
